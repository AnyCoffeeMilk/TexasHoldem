...
async function set_player_profile(name = null, avatar = null, bank = null, uuid = null) {
  ...
  if (avatar) {
    cookieStore.set("player_avatar", avatar);
  }
  if (bank) {
    cookieStore.set("player_bank", Math.min(Math.max(bank, 0), 99999).toString());
  }
  if (uuid) {
    cookieStore.set("player_uuid", uuid);
  }

  return { name, avatar, bank: Math.min(Math.max(bank, 0), 99999), uuid };
}
...
export { set_player_profile, read_player_profile, read_opponents_profile, get_opponent_action, add_bank };
